...
**ContextLease** is a provider-neutral Python framework for building prompts under a hard LLM context limit. It gives every prompt module a floor, target, and maximum budget; lets modules borrow unused capacity through revocable leases; and reclaims borrowed space through a pre-registered compression pipeline when another module needs it back.
...
See [CONTRIBUTING.md](CONTRIBUTING.md), [architecture](docs/architecture.md), and [provider guide](docs/providers.md).
